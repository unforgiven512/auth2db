...
if ($_SESSION["select_server"] AND $_SESSION["all_select_server"] != 1)
{
	...
	if (table_exists($_SESSION["ano"]."_".$t_mes."_".$t_dia) == 1) {
...
if ($_SESSION["select_tipo"] AND $_SESSION["all_select_tipo"] != 1)
{
...
if ($_SESSION["select_action"] AND $_SESSION["all_select_action"] != 1)
{
	...
	if (table_exists($_SESSION["ano"]."_".$t_mes."_".$t_dia) == 1) {
	    ...
	    $sql = "SELECT distinct action from log_0000_00_00";
	}
...
<tr><td valign="top">
	...
	<table width="200" border="0" cellpadding="1" cellspacing="1">
	  ...
	  <tr class="filas" valign="top" <? if ($rs->action != "") {
		...
		<td nowrap ><a href="auth2db_list.php?data=<?echo $rs->action?>&tipo=action " target="_blank" >
			<?
				$key = array_search(strtolower($rs->action), $array_action);
				//if (in_array(strtolower($rs->action), $array_action))
